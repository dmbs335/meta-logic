...
## Appendix C: Secure JWT Implementation Examples
...
# 1. Key generation (secure random)
...
def verify_token(token: str) -> dict:
    try:
        payload = jwt.decode(
            token,
            SECRET_KEY,
            algorithms=[ALGORITHM],  # Whitelist
            issuer=ISSUER,
            audience=AUDIENCE,
            options={
        ...
        # Additional validation: jti blacklist check
...
// 4. Header validation (kid etc.)
function validateHeader(token) {
  const decoded = jwt.decode(token, { complete: true });
  const header = decoded.header;

  // Block dangerous headers
  const blockedHeaders = ['jwk', 'jku', 'x5u', 'x5c'];
  for (const h of blockedHeaders) {
    if (header[h]) {
      throw new Error(`Header ${h} not allowed`);
    }
  }

  // kid validation (if used)
  if (header.kid && !/^[a-zA-Z0-9_-]{1,64}$/.test(header.kid)) {
...
public class JwtService {
    ...
    private void validateHeader(String token) {
        ...
        // Block dangerous headers
        if (header.containsKey("jwk") || header.containsKey("jku")
            || header.containsKey("x5u") || header.containsKey("x5c")) {
            throw new JwtException("Forbidden header parameter");
        }
    }

    private boolean isRevoked(String jti) {
        // Check blacklist via Redis etc.
        return false;
    }
}
```
